      ...
      if (subscription.subpath === "") {
        this.setVariableValues({
          [subscription.variableName]:
            typeof msgValue === "object" ? JSON.stringify(msgValue) : msgValue,
        });
      } else if (
        typeof msgValue === "object" &&
        objectPath.has(msgValue, subscription.subpath)
      ) {
  ...
  getConfigFields() {
    return [
      ...
      },
      {
        ...
        label: "Target URL",
        tooltip:
          "The URL of the WebSocket server (ws[s]://domain[:port][/path])",
        width: 12,
        regex: "/" + this.wsRegex + "/",
      },
      {
    ...
    this.setActionDefinitions({
      send_command: {
        ...
        callback: async (action, context) => {
          const value = await context.parseVariablesInString(
            action.options.data
          );
          if (this.config.debug_messages) {
